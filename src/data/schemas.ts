...
export const studentSchema = z.object({
    name: z.string().min(1),
    class: z.string().length(5).regex(classCheckPattern, "Invalid, Example: 11B04, 10a05")
}).strict();

export const repeatPasswordStr = z.string().min(6, "password too short")
export const passwordStr = repeatPasswordStr.refine(arg => {
    ...
    arg => { return { message: "Password too weak" } }
)

// Form
export const rawPermFormSchema = z.object({
    ...
    imgStr: z.string(),
    imgMatch: z.number().min(.6, "Match too low")
}).strict();
